...
zrequire('unread');
zrequire('common');
let search = zrequire('search_suggestion');
...
run_test('subset_suggestions', () => {
    let query = 'stream:Denmark topic:Hamlet shakespeare';
...
run_test('sent_by_me_suggestions', () => {
    ...
    let query = '';
    let suggestions = search.get_suggestions_legacy(query);
    assert(suggestions.strings.includes('sender:bob@zulip.com') !== -1);
...
run_test('whitespace_glitch', () => {
    let query = 'stream:office '; // note trailing space
